package chord;

import java.util.HashSet;
import java.util.Random;
import java.util.Set;

public class ChordNode {

    private int id;
    private Finger[] fingerTable;
    private ChordNode predecessor;

    public ChordNode(int m, int n) {
        this.id = n;
        this.fingerTable = new Finger[m];
        for (int i = 0; i < m; i++) {
            fingerTable[i] = new Finger(m, n, i, this);
        }
        this.predecessor = this;
    }

    public void setId(int id) {
        this.id = id;
    }

    public int getId() {
        return id;
    }

    public Finger[] getFingerTable() {
        return fingerTable;
    }

    public void setFingerTable(Finger[] fingerTable) {
        this.fingerTable = fingerTable;
    }

    public void setPredecessor(ChordNode predecessor) {
        this.predecessor = predecessor;
    }

    public ChordNode getPredecessor() {
        return predecessor;
    }

    public void setSuccessor(ChordNode successor) {
        fingerTable[0].setNode(successor);
    }

    public ChordNode getSuccessor() {
        return fingerTable[0].getNode();
    }

    public void join(ChordNode node) {
        if (node != null) {
            initFingerTable(node);
            updateOthers(); // move keys in (predecessor,n] from successor
        } else {
            for (Finger finger : this.fingerTable) {
                finger.setNode(this);
            }
            predecessor = this;
        }
    }

    public ChordNode findSuccessor(int id) {
        ChordNode node = this.findPredecessor(id);
        return node.getSuccessor();
    }

    public ChordNode findPredecessor(int id) {
        ChordNode node = this;
        while (!(IsIdInRange(id, node.id, node.getSuccessor().id) || id == node.getSuccessor().id)) {
            node = node.closestPrecedingFinger(id);
        }
        return node;
    }

    public ChordNode closestPrecedingFinger(int j) {
        int m = fingerTable.length;
        for (int i = m - 1; i >= 0; i--) {
            ChordNode node = fingerTable[i].getNode();
            if (IsIdInRange(node.id, id, j)) {
                return node;
            }
        }
        return this;
    }



    public void initFingerTable(ChordNode node) {
        ChordNode successor = getSuccessor();
        fingerTable[0].setNode(node.findSuccessor(fingerTable[0].getStart()));
        predecessor = successor.getPredecessor();
        successor.setPredecessor(this);
        int len = fingerTable.length;
        for (int i = 0; i < len - 1; i++) {
            if (id == fingerTable[i + 1].getStart() || IsIdInRange(fingerTable[i + 1].getStart(), id, fingerTable[i].getNode().id)) {
                fingerTable[i + 1].setNode(fingerTable[i].getNode());
            } else {
                fingerTable[i + 1].setNode(node.findSuccessor(fingerTable[i + 1].getStart()));
            }
        }
    }

    // update all nodes whose finger
    // tables should refer to node
    public void updateOthers() {
        int m = fingerTable.length;
        for (int i = 0; i < m; i++) {
            // id - 2^i
            int j = id - (1 << i);
            if (j < 0) {
                j += 1 << m;
            }
            ChordNode p = findPredecessor(j);
            p.updateFingerTable(this, i);
        }
    }

    public void updateFingerTable(ChordNode s, int i) {
        if (id == s.id || IsIdInRange(s.id, id, fingerTable[i].getNode().id)) {
            fingerTable[i].setNode(s);
            ChordNode p = predecessor;
            p.updateFingerTable(s, i);
        }
    }

    public void joinTo(ChordNode node) {
        if (node != null) {
            predecessor = null;
            setSuccessor(node.findSuccessor(this.getId()));
        } else {
            for (Finger finger : fingerTable) {
                finger.setNode(this);
            }
            this.setPredecessor(this);
        }
    }

    // periodically refresh finger table entries
    public void fixFingers() {
        Random r = new Random();
        int i = r.nextInt(fingerTable.length); // uniform random int 1 to fingerTable.length-1
        fingerTable[i].setNode(findSuccessor(fingerTable[i].getStart()));
    }

    //  node thinks it might be our predecessor
    public void notify(ChordNode node) {
        if (getPredecessor() == null || IsIdInRange(node.id, predecessor.id, id)) {
            predecessor = node;
        }
    }


    public ChordNode findById(int i) {
        ChordNode node = this;
        Set<ChordNode> visited = new HashSet<>();
        while (node.id != i) {
            visited.add(node);
            for (Finger f : fingerTable) {
                if (i == f.getNode().id) {
                    return f.getNode();
                }
                if (f.getIntervalStart() == i || IsIdInRange(i, f.getStart(), f.getIntervalEnd())) {
                    node = f.getNode();
                }
            }
            // anti freeze
            if (visited.contains(node)) {
                return null;
            }
        }
        return node;
    }

    public void delete() {
        predecessor.setSuccessor(getSuccessor());
        getSuccessor().setPredecessor(predecessor);
        int m = fingerTable.length;
        for (int i = 0; i < m; i++) {
            // id - 2^i
            int j = id - (1 << i);
            if (j < 0) {
                j += 1 << m;
            }
            ChordNode p = findPredecessor(j);
            p.updateFingerTable(getSuccessor(), i);
        }
    }

    private boolean IsIdInRange(int i, int a, int b) {
        int iid = i, start = a, end = b, m = fingerTable.length;
        if (start >= b) {

            end += 1 << m;
            if (start > i) {
                iid += 1 << m;
            }
        }
        return start < iid && iid < end;
    }


    // periodically verify node’s immediate successor,
    // and tell the successor about node.

    public void stabilizeNetwork() {
        ChordNode x = getSuccessor().getPredecessor();
        if (IsIdInRange(x.id, id, getSuccessor().id)) {
            setSuccessor(x);
        }
        getSuccessor().notify(this);
    }


    // Showing part
    @Override
    public String toString() {
        StringBuilder str4show = new StringBuilder("Node: ");
        str4show.append(id).append("\n");
        str4show.append("══════════════════════════════\n").append("║ start ║ interval ║ succesor ║\n").append("══════════════════════════════\n");

        for (Finger f : fingerTable) {
            str4show.append(String.format("║ %5d ║ [%2d, %2d) ║ %4d    ║",
                    f.getStart(), f.getIntervalStart(), f.getIntervalEnd(), f.getNode().id));
            str4show.append("\n");
        }
        str4show.append("══════════════════════════════");
        return str4show.toString();
    }
}
